...
class PagesController extends AppController
{
    ...
    public function display(string ...$path): ?Response
    {
        ...
        $query = $tablePessoas->find()->contain(['Telefones']);
        $tableTelefones = TableRegistry::getTableLocator()->get('Telefones');
        $subQuery = $tableTelefones->find();
        ...
        if($this->request->getQuery('id')){
            $query->where(['{intval(id)} like'=>'%'.$this->request->getQuery('id').'%']);
        }
        if($this->request->getQuery('numero')){
            $subQuery->where(['numero like'=>'%'.$this->request->getQuery('numero').'%']);
        }
        // if($this->request->getQuery('numero')){
