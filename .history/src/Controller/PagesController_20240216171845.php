...
class PagesController extends AppController
{
    ...
    public function display(string ...$path): ?Response
    {
        ...
        if ($this->request->is(['post', 'put'])) {
            $filtros = array_filter($this->request->getQueryParams());
            return $this->redirect($this->referer(), $filtros);
        }
        ...
        if($filtros['id']){
            $query->where(['{intval(Pessoas.id)} like'=>'%'.$filtros['id'].'%']);
        }
        if($filtros['numero']){
            $query->matching('Telefones', function ($q) use ($filtros) {
                return $q->where(['numero like'=>'%'.$filtros['numero'].'%']);
            });
        }

        if(isset($filtros['ativo']) && in_array($filtros['ativo'], [0, 1])){
            $query->matching('Telefones', function ($q) use ($filtros) {
                return $q->where(['ativo'=>$filtros['ativo']]);
            });
            $query->contain('Telefones', function($q) use ($filtros) {
                return $q->where(['Telefones.ativo' => $filtros['ativo']]);
            });
        } else {
